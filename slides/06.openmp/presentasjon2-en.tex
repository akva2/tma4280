...
\frametitle{MPI - program flow}
	\begin{figure}[H]
		\includegraphics[width=8cm]{../../notes/03.openmp/mpi}
	\end{figure}
	\begin{itemize}
...
\frametitle{OpenMP - program flow}
	\begin{figure}[H]
		\includegraphics[width=8cm]{../../notes/03.openmp/openmp}
	\end{figure}
	\begin{itemize}
...
\frametitle{How to use OpenMP in your code}
	\begin{itemize}
		...
		\item Consider the serial snippet \lstinputlisting[language=C]{../../notes/03.openmp/serial-for.c}
	\end{itemize}
}
...
\frametitle{OpenMP - loop example - each iteration has a fixed cost}
	\begin{itemize}
	    ...
	    \item To divide this loop among several threads using OpenMP you simply do \lstinputlisting[language=C]{../../notes/03.openmp/openmp-for.c}
...
\frametitle{OpenMP - loop example - each iteration has differing costs}
	\begin{itemize}
	    ...
	    \item To divide this loop among several threads using OpenMP you do \lstinputlisting[language=C]{../../notes/03.openmp/openmp-for-dynamic.c}
...
\frametitle{OpenMP - loop example - each iteration has differing costs}
	\begin{itemize}
		...
		\item We can try to minimize this problem by specifying a \emph{chunk size};
			\lstinputlisting[language=C]{../../notes/03.openmp/openmp-for-dynamic-chunk.c}
	\end{itemize}
}
...
\frametitle{OpenMP - loop example - each iteration has differing costs}
	\begin{itemize}
		...
		\item OpenMP offers a third scheduling mode which attempts to
			...
			\lstinputlisting[language=C]{../../notes/03.openmp/openmp-for-guided-chunk.c}
	\end{itemize}
}
\frame[label=overview8]{
\frametitle{OpenMP - sections}
	\begin{itemize}
		\item Consider the serial snippet \lstinputlisting[language=C]{../../notes/03.openmp/serial-sections.c}
		\item We here assume that the jobs are independent and share no resources.
		\item To execute these jobs in parallel on several threads you do \lstinputlisting[language=C,basicstyle=\tiny]{../../notes/03.openmp/openmp-sections.c}
	\end{itemize}
}
\frame[label=example1]{
	...
	\begin{itemize}
		\item We here consider a simple example which computes
			\[
				\alpha=\sum_{i=1}^{K} \mathbf{u}_i^T\mathbf{A}\mathbf{u}_i.
			\]
...
\frame[label=example2]{
	...
	\lstinputlisting[language=C]{../../notes/03.openmp/serial-mxv.c}
}
\frame[label=example21]{
	\frametitle{Simple example - serial contd.}
	\lstinputlisting[language=C]{../../notes/03.openmp/serial-mxv-2.c}
}
...
\frame[label=example4]{
	...
	\lstinputlisting[language=C,basicstyle=\small]{../../notes/03.openmp/openmp-mxv.c}
}
...
\frame[label=example6]{
	...
	\lstinputlisting[language=C,basicstyle=\small]{../../notes/03.openmp/serial-mxv-ref.c}
}
\frame[label=example7]{
	\frametitle{Simple example - OpenMPI version 2}
	\lstinputlisting[language=C,basicstyle=\small]{../../notes/03.openmp/openmp-mxv-ref.c}
}
\frame[label=example8]{
	\frametitle{Simple example - MPI}
	\lstinputlisting[language=C,basicstyle=\small]{../../notes/03.openmp/mpi-mxv.c}
	\begin{itemize}
...
\frame[label=howto1]{
	\frametitle{OpenMP - how to enable the feature on Vilje}
	\begin{itemize}
		\item On Vilje you compile your C programs using
			\lstinputlisting[language=sh]{../../notes/03.openmp/compile-vilje}
		\item If you compile OpenMP code without
			the switches, the compiler will warn you that it encountered pragma's it did not understand.
...
\frame[label=howto2]{
	...
	\begin{itemize}
		\item On a GNU system you compile your C programs using
			\lstinputlisting[language=sh]{../../notes/03.openmp/compile-gnu}
...
\frame[label=howto3]{
	\frametitle{OpenMP - how to enable using CMake}
	\begin{itemize}
          \item Of course we do not want to deal with these differences, and
                let CMake handle it for us.
          \item We simply add two lines;
            \lstinputlisting{cmake-openmp.txt}
          \item Notice that we do not use the REQUIRED keyword.
	\end{itemize}
}
\frame[label=speedup]{
