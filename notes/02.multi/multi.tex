...
Supercomputing represents the high performance segment of the overall computing market. This segment has traditionally been driven by grand challenge problems 
...
%\clearpage
\section{The previous supercomputer at NTNU}

We now give a brief overview of the previous supercomputer at NTNU, 
an IBM supercomputer based on the POWER5 microprocessor. 

\subsection{The POWER5 microprocessor}

Multi-core systems is a recent trend in chip design. 
The POWER5 microprocessor represents an example of a dual-core chip. 
This means that a single chip integrates two processors (or cores) 
into a single integrated circuit; see Figure \ref{fig:p5}.
A single chip therefore represents two physical processors in the 
sense discussed earlier. 

There are also other important differences compared to the prototypical 
RISC processor we discussed initially. 
One difference is that the two processors in a single POWER5 chip 
share a single L2 cache. They also share the interconnect to the rest of 
the system, including the memory modules. However, 
the L1 data and instruction caches are private to each processor (as before). 

A new development compared to earlier models of the POWER-family 
of microprocessors is the introduction of a feature called {\em multi-threading}. 
This means that each of the two processors
(or cores) is able to handle two instruction streams at the same time. 
This is also referred to as simultaneous multi-threading (SMT). 
Overall this means that a single POWER5 chip with two physical processors can 
handle four logical processes (or instruction streams) at the same time. 
The POWER5 system can operate in either a single-threaded or a 
multi-threaded execution mode. The motivation for introducing a multi-threaded mode
is to  be able to better exploit the resources in a processor/CPU, 
i.e., reduce the time that any of the functional units remain idle. 
The performance enhancement seen using multi-threading will depend 
on the the particular program, e.g., the associated memory traffic. 

\begin{figure}[htbp]
  \begin{center}
    \includegraphics[scale=0.5]{P5}
  \end{center}
\caption{The figure depicts the POWER5 microprocessor (or chip). 
}
\label{fig:p5}
\end{figure}
...
Multiple POWER5 chips can be connected to form larger modules. 
A multichip module (MCM) comprises four POWER5 chips and four L3 chips. 
Two MCMs comprise what is referred to as a {\em node}; see Figure \ref{fig:node}. 
A node also represents a symmetric multiprocessor (SMP), which means that 
the aggregate memory within a node is available to all the processors (or cores). 
Hence, a single node can be used as a shared memory system. 

\begin{figure}[htbp]
  \begin{center}
    \includegraphics[scale=0.4]{node}
  \end{center}
\caption{A node based is based on two multichip modules, i.e., 
eight POWER5 chips or 16 phyisical processors (or cores). 
In a multi-threading execution mode, 
a node thus represents 32 logical processors. 
}
\label{fig:node}
\end{figure}
...
\subsection{Key data for \texttt{njord}}

The current supercomputer at NTNU, \texttt{njord}, is based on 192 nodes. 
 The total number of processors (or cores) is therefore 3072. 
 
Each node represents a shared memory system with 8 dual-core POWER5 chips
which share 32 GB memory (although a few nodes have 128 GB memory). 

Each POWER5 processor operates at a clock rate of 1.9 GHz. 
The size of the L1 cache is 32 kbyte for data and 64 kbyte for instructions. 
The size of the shared L2 cache is 1.9 Mbyte, while the size of the 
off-chip L3 cache is 36 Mbyte. 

The latency associated with the different memory levels is: 
12 clock cycles for L2, 80 clock cycles for L3, and 220 clock cycles for main memory.

\section{Programming models}

We have seen that a node (with 16 physical processors) on \texttt{njord} 
represents a shared memory system where the aggregate memory for the node 
is globally available to all the 16 processors. 
A shared memory programming model (i.e., OpenMP) 
...
system, the message-passing programming model may still be used within a node. However, the opposite is not true: a shared programming model 
cannot be used on a "pure" distributed memory system 
(e.g., on a PC cluster). 

Note that a system like \texttt{njord}, which represents a shared memory 
